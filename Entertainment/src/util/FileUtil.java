...
public class FileUtil {
	...
	public int upload(HttpServletRequest request,String uploadPath) throws IOException{
		//实例化一个硬盘文件，用来配置上传组建ServletFileUpload
		DiskFileItemFactory diskFileItemFactory =new DiskFileItemFactory();
		//设置上传文件时用于临时存放文件的内存大小，这里是4KB,多于的部分将临时存在硬盘
		...
		while(fileItr.hasNext()){
			...
			if(!fileItem.isFormField()){
				...
				if(!sourceFilePath.equals("")&&size!=0){
					...
					try{
						fileItem.write(new File(realPath));//这里的File是什么
					}catch(Exception e){
						e.printStackTrace();
						return 0;
					}
				...
				//如果不是上传文件而是表单信息，则将信息保存在parameter中
