...
<body>
	<center>
		<h2>分类显示</h2>
		<c:set var="url" value="${pageContext.request.contextPath}/us/sort"></c:set>
		...
		每页 ${pageBean.pageSize}行 共${pageBean.totals}行 页数${pageBean.curPage}/${pageBean.totalPage}
		<c:choose>
		...
		<c:choose>
			<c:when test="${pageBean.curPage>=pageBean.totals}">下一页 尾页</c:when>
			<c:otherwise>
